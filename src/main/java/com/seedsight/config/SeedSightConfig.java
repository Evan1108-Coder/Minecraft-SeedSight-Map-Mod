...
public class SeedSightConfig {
    ...
    public static SeedSightConfig load() {
        Path configPath = getConfigPath();
        if (Files.exists(configPath)) {
            try {
                String json = Files.readString(configPath);
                SeedSightConfig cfg = GSON.fromJson(json, SeedSightConfig.class);
                if (cfg != null) {
                    cfg.validate();
                    return cfg;
                }
            } catch (IOException e) {
                SeedSightClient.LOGGER.warn("Failed to load config, using defaults", e);
            }
        }
        SeedSightConfig cfg = new SeedSightConfig();
        cfg.save();
        return cfg;
    }
    ...
    public void save() {
        ...
        try {
            ...
            SeedSightClient.LOGGER.warn("Failed to save config", e);
        }
    }

    private static Path getConfigPath() {
        return FabricLoader.getInstance().getConfigDir().resolve("seedsight.json");
    }
